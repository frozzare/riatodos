...
var Brocco = (function () {
  ...
  function generateDocumentation(source, config, callback) {
    ...
    var renderSections = function (sections) {
      ...
      sections.forEach(function (section) {
        if (section.docsText && !section.docsHtml) section.docsHtml = showdown.makeHtml(section.docsText);
    ...
    if (!callback) callback = insertHtmlIntoBody;
    ...
    // example.
    if (language.makeSections) parseAndHighlight = function () {
  ...
  var path = {
    basename: function (p, ext) {
      ...
      if (ext) lastPart = lastPart.slice(0, -(ext.length));
      return lastPart;
    },
  ...
  function codeMirrorHighlighter(language, fragments, cb) {
    if (typeof (CodeMirror) == "undefined") return nullHighlighter(language, fragments, cb);

    var mode = CodeMirror.getMode(CodeMirror.defaults, {
      name: language.name
    });
    if (mode.name == "null") return nullHighlighter(language, fragments, cb);
    ...
    var onText = function (text, style) {
      ...
      if (style) {
        if (codeMirrorStyleMap[style]) style = codeMirrorStyleMap[style] + " cm-" + style;
        else style = "cm-" + style;
    ...
    fragments = accum.join("")
      .split('\n<span class="c cm-comment">' + language.symbol + 'DIVIDER</span>\n');
  ...
  function defaultTemplate(context) {
    function el(name, attrs, children) {
      ...
      (children || []).forEach(function (child) {
        if (typeof (child) == "undefined") child = "undefined";
  ...
  function getSourceFile(filename, cb) {
    ...
    req.onerror = function () {
      ...
      if (location.protocol == "file:") lines = lines.concat([
